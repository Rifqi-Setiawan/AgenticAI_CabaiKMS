...
def _annotation_row(annotation: GoldMappingAnnotation) -> dict:
...
def finalize_campaign(
    ...
    frame = pd.DataFrame(
        [_annotation_row(record.final_annotation) for record in merged],
    ...
    history_output.write_text(
        json.dumps([record.model_dump(mode="json") for record in merged], ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
